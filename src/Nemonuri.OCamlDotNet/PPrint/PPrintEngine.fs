(**
    - Original source: https://github.com/fpottier/pprint/blob/20230830/src/PPrintEngine.ml
    - Modifier: Nemonuri
*)
...
module PPrintEngine =
    ...
    type buffering (``delegate`` : output) as self =
        class
            ...
            interface output with
                member this.char c = this.char c
                member this.substring s ofs len = this.substring s ofs len
        end
    ...
    type channel_output (channel: OCamlOutChannel) = 
        class
            member _.char = output_char channel
            member _.substring = output_substring channel
    ...
    type buffer_output (buffer: Buffer.t) =
        class
            member _.char = Buffer.add_char buffer
            member _.substring = Buffer.add_substring buffer
            interface output with
                member this.char c = this.char c
                member this.substring s ofs len = this.substring s ofs len
        end
    ...
    let initial rfrac width = {
        width = width;
        ribbon = max 0 (min width (Int.of_float (Int.to_float width *. rfrac)));
        last_indent = 0;
        line = 0;
        column = 0
    }
    ...
    type custom =
        interface
            ...
            abstract member requirement: requirement
    ...
    type document =
        ...
        | FancyString of OCamlString * int * int * int
    ...
    and private ``continue`` output state = function
        | KNil ->
                ()
    ...
    let rec private compact' (output: output) doc cont =
        match doc with
        | Empty ->
    ...
    and private continue' output cont =
        match cont with
        | [] ->
                ()
        | doc :: cont ->
