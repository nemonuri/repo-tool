...
// reference: https://github.com/FStarLang/FStar/blob/v2025.12.15/src/basic/FStarC.Util.fsti
// reference: https://github.com/FStarLang/FStar/blob/v2025.12.15/src/ml/FStarC_Util.ml
...
module Util =
    ...
    *)
    ...
    let max_int = Z.of_int Int.max_int
    let is_letter c = if c > 255 then false else Char.Ascii.is_letter (Char.chr c)
    let is_digit  c = if c > 255 then false else Char.Ascii.is_digit  (Char.chr c)
    let is_letter_or_digit c = is_letter c || is_digit c
    let is_symbol c = if c > 255 then false else BatChar.is_symbol (Char.chr c)
...
#endif
    let split s sep = BatString.split_on_string sep s
    let splitlines s = split s (toString "\n"B)
#if false
