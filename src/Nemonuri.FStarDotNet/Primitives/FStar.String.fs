// Reference: https://github.com/FStarLang/FStar/blob/v2025.12.15/fsharp/base/FStar_String.fs
...
let make (i : nat) (c : FStar.Char.char) = String.init (Microsoft.FSharp.Core.Operators.int i) (fun _ -> string([|c|]))
let strcat (s: string) (t: string) : string = Core.Operators.(+) s t
let op_Hat s t =  strcat s t

let split (seps : FStar.Char.char list) (s : string) = s.Split(Array.ofList seps) |> Array.toList
let inline split' seps (s: string) = split (CharListTheory.ToCharList' seps) s
